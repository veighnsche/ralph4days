...
//    - Read-only access to current state via SQLite queries
...
while IFS= read -r line; do
    ...
    if [[ "$line" == *'"initialize"'* ]]; then
        ...
        if [[ "$line" == *'"list_features"'* ]]; then
            content=$(json_escape "$(sqlite3 "$RALPH_DB" "SELECT name, display_name, acronym, description FROM features ORDER BY name;" 2>/dev/null || echo "No features found")")
            printf '{{"jsonrpc":"2.0","id":%s,"result":{{"content":[{{"type":"text","text":"%s"}}]}}}}\n' "$id" "$content"
        elif [[ "$line" == *'"list_disciplines"'* ]]; then
            content=$(json_escape "$(sqlite3 "$RALPH_DB" "SELECT name, display_name, icon, color FROM disciplines ORDER BY name;" 2>/dev/null || echo "No disciplines found")")
            printf '{{"jsonrpc":"2.0","id":%s,"result":{{"content":[{{"type":"text","text":"%s"}}]}}}}\n' "$id" "$content"
        elif [[ "$line" == *'"read_tasks"'* ]]; then
            content=$(json_escape "$(sqlite3 -header -column "$RALPH_DB" "SELECT id, feature, discipline, title, status, priority FROM tasks ORDER BY id;" 2>/dev/null || echo "No tasks found")")
            printf '{{"jsonrpc":"2.0","id":%s,"result":{{"content":[{{"type":"text","text":"%s"}}]}}}}\n' "$id" "$content"
        fi
...
# Provides tools + resources for read access to .ralph/db/ralph.db
RALPH_DB='{db_path}/ralph.db'

{id_extractor}

# Query a SQLite table and return results
query_table() {{
    local table="$1"
    sqlite3 -header -column "$RALPH_DB" "SELECT * FROM $table ORDER BY 1;" 2>/dev/null || echo "Query failed"
}}
...
while IFS= read -r line; do
    ...
    if [[ "$line" == *'"initialize"'* ]]; then
        ...
        printf '{{"jsonrpc":"2.0","id":%s,"result":{{"tools":[{{"name":"query_db","description":"Run a read-only SQL query on the ralph database","inputSchema":{{"type":"object","properties":{{"sql":{{"type":"string","description":"SQL SELECT query to run"}}}}}}}},{{"name":"list_tables","description":"List all tables in the database","inputSchema":{{"type":"object","properties":{{}}}}}}]}}}}\n' "$id"
    elif [[ "$line" == *'"tools/call"'* ]]; then
        if [[ "$line" == *'"query_db"'* ]]; then
            sql="${{line#*\"sql\":\"}}"
            sql="${{sql%%\"*}}"
            content=$(json_escape "$(sqlite3 -header -column "$RALPH_DB" "$sql" 2>&1)")
            printf '{{"jsonrpc":"2.0","id":%s,"result":{{"content":[{{"type":"text","text":"%s"}}]}}}}\n' "$id" "$content"
        elif [[ "$line" == *'"list_tables"'* ]]; then
            content=$(json_escape "$(sqlite3 "$RALPH_DB" ".tables" 2>/dev/null)")
            printf '{{"jsonrpc":"2.0","id":%s,"result":{{"content":[{{"type":"text","text":"%s"}}]}}}}\n' "$id" "$content"
        fi
    elif [[ "$line" == *'"resources/list"'* ]]; then
        printf '{{"jsonrpc":"2.0","id":%s,"result":{{"resources":[{{"uri":"ralph://db/tasks","name":"tasks","mimeType":"text/plain"}},{{"uri":"ralph://db/features","name":"features","mimeType":"text/plain"}},{{"uri":"ralph://db/disciplines","name":"disciplines","mimeType":"text/plain"}},{{"uri":"ralph://db/metadata","name":"metadata","mimeType":"text/plain"}}]}}}}\n' "$id"
    elif [[ "$line" == *'"resources/read"'* ]]; then
        uri="${{line#*\"uri\":\"}}"
        uri="${{uri%%\"*}}"
        table="${{uri#ralph://db/}}"
        content=$(json_escape "$(query_table "$table")")
        printf '{{"jsonrpc":"2.0","id":%s,"result":{{"contents":[{{"uri":"%s","mimeType":"text/plain","text":"%s"}}]}}}}\n' "$id" "$uri" "$content"
    elif [[ "$line" == *'"notifications/"'* ]]; then
        : # notifications require no response
    fi
done
